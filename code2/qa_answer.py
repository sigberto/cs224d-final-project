...
tf.app.flags.DEFINE_integer("num_perspectives", 30, "Dimension of perspectives matrix in BMPM.")
tf.app.flags.DEFINE_integer("num_perspective_classes", 6, "Dimension of perspectives matrix in BMPM.")
tf.app.flags.DEFINE_string("data_dir", "data/squad", "SQuAD directory (default ./data/squad)")
tf.app.flags.DEFINE_string("train_dir", "weights_65", "Training directory to save the model parameters (default: ./train).")
...
tf.app.flags.DEFINE_string("max_paragraph_size", 300, "Path to the trimmed GLoVe embedding (default: ./data/squad/glove.trimmed.{embedding_size}.npz)")
tf.app.flags.DEFINE_string("max_question_size", 25, "Path to the trimmed GLoVe embedding (default: ./data/squad/glove.trimmed.{embedding_size}.npz)")
tf.app.flags.DEFINE_string("model", "bmpm", "Which baseline should we use")
tf.app.flags.DEFINE_string("testing", "train", "Run on same batch.")
tf.app.flags.DEFINE_integer("validation_sample", 200, "Sample size for validation batch.")
...
def generate_answers(sess, model, dataset, rev_vocab):
    ...
    while True:
        if idx >= len(contexts): break
        paragraph_batch = contexts[idx:idx+FLAGS.batch_size]
        question_batch = questions[idx:idx+FLAGS.batch_size]
        uuid_batch = question_uuids[idx:idx+FLAGS.batch_size]
        
        #paragraphs, paragraph_masks = model.mask_and_pad(paragraph_batch, 'paragraph')
        #questions, question_masks = model.mask_and_pad(question_batch, 'question')
        #a_s, a_e = model.answer(sess, paragraphs, paragraph_masks, questions, question_masks)
        a_s, a_e = model.answer(sess, paragraph_batch, question_batch)
        
        for i in range(len(uuid_batch)):
            uuid = uuid_batch[i]
            context = paragraph_batch[i]
...
def main(_):
    ...
    # ========= Load Dataset =========
    # You can change this code2 to load dataset in your own way
