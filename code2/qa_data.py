...
def process_glove(args, vocab_list, vocab, save_path, size=4e5, random_init=True):
    """
...
def create_vocabulary_orig(vocabulary_path, data_paths, tokenizer=None):
            ...
            with open(path, mode="rb") as f:
                if 'glove' not in path:
                    ...
                    for line in f:
                        array = line.lstrip().rstrip().split(" ")
                        w = array[0]
                        glove_vocab[w] = id_num
                        glove_vocab[w.capitalize()] = id_num + 1
                        id_num += 2
        for word in vocab:
            if word in glove_vocab:
                del glove_vocab[word]
        vocab_list = _START_VOCAB + sorted(vocab, key=vocab.get, reverse=True) + sorted(glove_vocab, key=glove_vocab.get, reverse=False) 
        print("Vocabulary size: %d" % len(vocab_list))
        with gfile.GFile(vocabulary_path, mode="wb") as vocab_file:
            for w in vocab_list:
                vocab_file.write(w + b"\n")
...
if __name__ == '__main__':
    ...
    create_vocabulary(vocab_path,
                      [pjoin(args.source_dir, "train.context"),
                       ...
                       pjoin(args.glove_dir, "glove.6B.100d.txt")])
    ...
    process_glove(args, rev_vocab, vocab, args.source_dir + "/glove.trimmed.{}".format(args.glove_dim),
