...
class QASystem(object):
    ...
    def setup_system_bmpm_j(self):
        ...
        # step 3
        (knowledge_rep_s_fw, knowledge_rep_s_bw), (_, _) = self.encoder.encode_bilstm(m_p, self.paragraph_mask, scope='knowledge_rep_1')
        knowledge_rep_s = tf.concat(2, [knowledge_rep_s_fw, knowledge_rep_s_bw])
        self.a_s = self.decoder.decode_matrix(knowledge_rep_1, self.FLAGS.state_size, scope='answer_start')

        # step 4
        (knowledge_rep_e_fw, knowledge_rep_e_bw), (_, _) = self.encoder.encode_gru(knowledge_rep_s, self.paragraph_mask, scope='knowledge_rep_2')
        knowledge_rep_e = tf.concat(2, [knowledge_rep_e_fw, knowledge_rep_e_bw])
        self.a_e = self.decoder.decode_matrix(knowledge_rep_e, self.FLAGS.state_size, scope='answer_end')
